...
namespace BlueSky.Pages
{
  public partial class MainPage : Page, INotifyPropertyChanged
  {
    ...
    private Core.Lexicons.App.BSky.Actor.Defs.SavedFeedsPrefV2 _savedFeedsPrefV2;
    ...
    public int UnreadConvoNotificationCount
        { get { return _unreadConvoCount; } 
            set { _unreadConvoCount = value; 
                OnPropertyChanged(nameof(UnreadConvoNotificationCount)); 
                OnPropertyChanged(nameof(UnreadConvoNotificationCountVisibility)); 
            } 
        }
    public Visibility UnreadConvoNotificationCountVisibility
        { get { return _unreadConvoCount > 0 
                    ? Visibility.Visible
                    : Visibility.Collapsed; } }
    ...
    {
      var preferences = await Get<Core.Lexicons.App.BSky.Actor.GetPreferences.Response>
                (new Core.Lexicons.App.BSky.Actor.GetPreferences());
    ...
    {
      _savedFeedsPrefV2 = _preferences?.FirstOrDefault(s => s
      is Core.Lexicons.App.BSky.Actor.Defs.SavedFeedsPrefV2) 
                as Core.Lexicons.App.BSky.Actor.Defs.SavedFeedsPrefV2;

      var getFeedReq = new Core.Lexicons.App.BSky.Feed.GetFeedGenerators();
          ...
          case "feed":
            {
              ...
              var name = feedURI.Contains("/") ? feedURI.Substring(
                  feedURI.LastIndexOf("/") + 1) : feedURI; // temp name
      ...
      var feedInfoResponse = await Get<Core.Lexicons.App.BSky.Feed.GetFeedGenerators.Response>(getFeedReq);
    ...
    {
      var unreadCountResponse = await Get<Core.Lexicons.App.BSky.Notification.GetUnreadCount.Response>(
          new Core.Lexicons.App.BSky.Notification.GetUnreadCount());
      if (unreadCountResponse != null)
      {
        UnreadNotificationCount = (int)unreadCountResponse.count;
      }

      var convoUnreadCountResponse = await Get<Core.Lexicons.Chat.BSky.Convo.ListConvos.Response>
                (new Core.Lexicons.Chat.BSky.Convo.ListConvos() {
        limit = 1
      });
    ...
    {
      string atUri = await Core.Helpers.HTTPToATURI(_app.Client, postHttpURI);
      if (atUri != null)
      {
    ...
    public void Reply(Core.Lexicons.App.BSky.Feed.Defs.PostView post)
    {
    ...
    public void Quote(Core.Lexicons.App.BSky.Feed.Defs.PostView post)
    {
      var postInlay = PostInlay as Inlays.NewPostInlay;
      if (postInlay != null)
      {
    ...
    public async Task<T> Get<T>(Core.ILexiconRequest input) where T : class, Core.ILexiconResponse
    {
      try
      {
    ...
    public async Task<T> Post<T>(Core.ILexiconRequest input) where T : class, Core.ILexiconResponse
    {
      try
      {
    ...
    public class Feed : INotifyPropertyChanged
    {
      ...
      public Core.Lexicons.App.BSky.Feed.Defs.GeneratorView FeedInfo { get; set; } = null;
