...
namespace BlueSky.Inlays
{
  public partial class ThreadInlay : UserControl, INotifyPropertyChanged
  {
    ...
    public ObservableCollection<Core.Lexicons.App.BSky.Feed.Defs.PostView> Posts { get; set; }
    ...
    public async Task Refresh()
    {
      ...
      var response = await _mainPage.Get<Core.Lexicons.App.BSky.Feed.GetPostThread.Response>
                (new Core.Lexicons.App.BSky.Feed.GetPostThread()
      {
        uri = PostURI
      });
      var thread = response?.thread as Core.Lexicons.App.BSky.Feed.Defs.ThreadViewPost;
      if (thread != null)
      {
        // main post
        Posts = new ObservableCollection<Core.Lexicons.App.BSky.Feed.Defs.PostView>();
        Posts.Add(thread.post);

        // previous posts
        var post = thread.parent as Core.Lexicons.App.BSky.Feed.Defs.ThreadViewPost;
        while (post != null)
        {
          Posts.Insert(0, post.post);
          post = post?.parent as Core.Lexicons.App.BSky.Feed.Defs.ThreadViewPost;
        }
        ...
        if (thread.replies != null)
        {
          ...
          {
            var replyPost = reply as Core.Lexicons.App.BSky.Feed.Defs.ThreadViewPost;
