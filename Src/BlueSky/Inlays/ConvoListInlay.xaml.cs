...
namespace BlueSky.Inlays
{
  public partial class ConvoListInlay : UserControl, INotifyPropertyChanged
  {
    ...
    public async Task Refresh()
    {
      ...
      var response = await _mainPage.Get<Core.Lexicons.Chat.BSky.Convo.ListConvos.Response>(
    ...
    public class Convo
    {
      public Core.Lexicons.Chat.BSky.Convo.Defs.ConvoView ConvoView 
            { 
                get; 
                set; 
            }
      public string CurrentUserDID 
            { 
                get; 
                set; 
            }
      ...
      public string PartnerAvatarURL
                => ConvoView.members.Where(s=>s.did != CurrentUserDID).FirstOrDefault().avatar;
      public string PartnerNames 
                => string.Join(", ", ConvoView.members.Where(
                    s => s.did != CurrentUserDID).Select(s => s.DisplayName));
      public string LastMessage 
                => (ConvoView.lastMessage as Core.Lexicons.Chat.BSky.Convo.Defs.MessageView)?.text
                ?? string.Empty;
