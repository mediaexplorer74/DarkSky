...
namespace BlueSky.Controls.Post
{
  public class PostBase : UserControl, INotifyPropertyChanged
  {
    ...
    public IEnumerable<Core.Lexicons.App.BSky.Embed.Images.ViewImage> PostImages 
            => PostView?.PostImages;
    public Core.Lexicons.App.BSky.Embed.External.View PostEmbedExternal 
            => PostView?.PostEmbedExternal;
    public Core.Lexicons.App.BSky.Embed.Record.ViewRecord QuotedPost 
            => PostView?.QuotedPost;
    public Core.Lexicons.App.BSky.Embed.Video.View PostVideo => PostView?.PostVideo;
    ...
    public int VideoHeight
    {
      get
      {
        ...
        int height = (int)(ActualWidth * PostVideo.aspectRatio.height
                                           / PostVideo.aspectRatio.width);
        height = Math.Min(height, 400);
        return height;
      }
    }

    public Core.IPost PostData
    {
      get { return GetValue(PostDataProperty) as Core.IPost; }
      set { SetValue(PostDataProperty, value); }
    }
    public static readonly DependencyProperty PostDataProperty 
            = DependencyProperty.Register("PostData", typeof(Core.IPost), 
                typeof(PostBase), new PropertyMetadata(null, OnPostDataChanged));

    public Core.Lexicons.App.BSky.Feed.Defs.FeedViewPost FeedViewPost
            => PostData as Core.Lexicons.App.BSky.Feed.Defs.FeedViewPost;
    public Core.Lexicons.App.BSky.Feed.Defs.PostView PostView
    {
      get
      {
        var feedViewPost = PostData as Core.Lexicons.App.BSky.Feed.Defs.FeedViewPost;
        if (feedViewPost != null)
        {
          return feedViewPost?.post;
        }

        return PostData as Core.Lexicons.App.BSky.Feed.Defs.PostView;
      }
    }

    protected static void OnPostDataChanged(DependencyObject d, 
    ...
    protected async void Repost_Click(object sender, RoutedEventArgs e)
    {
      ...
      {
        // add repost
        var response = await _mainPage.Post<Core.Lexicons.COM.ATProto.Repo.CreateRecord.Response>(
            new Core.Lexicons.COM.ATProto.Repo.CreateRecord()
        {
          repo = _app.Client.DID,
          collection = "app.bsky.feed.repost",
          record = new Core.Lexicons.App.BSky.Feed.Repost()
          {
            createdAt = DateTime.Now,
            subject = new Core.Lexicons.COM.ATProto.Repo.StrongRef()
            {
              cid = PostView.cid,
              uri = PostView.uri,
            }
      ...
      else
      {
        ...
        if (Core.Helpers.ParseATURI(PostView?.viewer?.repost, ref repo, ref collection, ref rkey))
        {
          var response = await _mainPage.Post<Core.Lexicons.COM.ATProto.Repo.DeleteRecord.Response>
                        (new Core.Lexicons.COM.ATProto.Repo.DeleteRecord()
          {
            repo = repo,
    ...
    protected async void Like_Click(object sender, RoutedEventArgs e)
    {
      if (!PostLiked)
      {
                    ...
                    (new Core.Lexicons.COM.ATProto.Repo.CreateRecord()
        {
          ...
          record = new Core.Lexicons.App.BSky.Feed.Like()
          {
            createdAt = DateTime.Now,
            subject = new Core.Lexicons.COM.ATProto.Repo.StrongRef()
            {
      ...
      else
      {
        ...
        string collection = string.Empty;
        string rkey = string.Empty;
        if (Core.Helpers.ParseATURI(PostView?.viewer?.like, ref repo, ref collection, ref rkey))
        {
          var response = await _mainPage.Post<Core.Lexicons.COM.ATProto.Repo.DeleteRecord.Response>(
              new Core.Lexicons.COM.ATProto.Repo.DeleteRecord()
          {
    ...
    protected void MiscCopy_Click(object sender, RoutedEventArgs e)
    {
      ...
      string collection = string.Empty;
      string rkey = string.Empty;
      if (Core.Helpers.ParseATURI(PostView.uri, ref repo, ref collection, ref rkey))
      {
    ...
    protected List<Inline> GenerateInlines()
    {
      ...
      var text = PostText;
      var facets = (PostView?.record as Core.Lexicons.App.BSky.Feed.Post)?.facets;
      ...
      foreach (var index in indices)
      {
        ...
        {
          string textFragment = System.Text.Encoding.UTF8.GetString(bytes, (int)current, 
              (int)(index.byteStart - current));
        ...
        string linkText = System.Text.Encoding.UTF8.GetString(bytes, (int)index.byteStart, 
            (int)(index.byteEnd - index.byteStart));

        if (facet.features[0] as Core.Lexicons.App.BSky.RichText.Facet.Link != null)
        {
          var link = facet.features[0] as Core.Lexicons.App.BSky.RichText.Facet.Link;
          var hyperlink = new Hyperlink();
          hyperlink.NavigateUri = new Uri(link.uri);
          hyperlink.Inlines.Add(new Run() { Text = linkText });
          result.Add(hyperlink);
        }
        else if (facet.features[0] as Core.Lexicons.App.BSky.RichText.Facet.Mention != null)
        {
          var mention = facet.features[0] as Core.Lexicons.App.BSky.RichText.Facet.Mention;
          var hyperlink = new Hyperlink();
      ...
      {
        var textFragment = System.Text.Encoding.UTF8.GetString(bytes, 
            (int)current, bytes.Length - (int)current);
    ...
    protected void MediaPlayerElement_Loaded(object sender, Windows.UI.Xaml.RoutedEventArgs e)
    {
      ...
      var video = PostView.embed as Core.Lexicons.App.BSky.Embed.Video.View;
      if (video == null)
      {
        return;
      }
